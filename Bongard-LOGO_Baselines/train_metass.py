...
def main(config):
    ...
    for epoch in range(1, max_epoch + 1):
        ...
        for data, label in tqdm(train_loader, desc='train', leave=False):
            ...
            logits, sslogits = model(x_shot, x_query)
            logits = logits.view(-1, n_train_way)
            ce_loss = F.cross_entropy(logits, label_query)
            acc = utils.compute_acc(logits, label_query)
            
            sslabel = torch.zeros(sslogits.shape[0], n_train_way, n_train_shot, dtype=torch.uint8, device=sslogits.device)
            sslabel[:,1] = 1
            sslabel = sslabel.view(-1)
            sslogits = sslogits.view(-1, n_train_way)
            ssce_loss = F.cross_entropy(sslogits, sslabel)
            
            loss = ce_loss + 0.5*ssce_loss
        ...
        for name, loader, name_l, name_a in [('val', val_loader, 'vl', 'va')] + tval_tuple_lst:
            ...
            for data, _ in tqdm(loader, desc=name, leave=False):
                ...
                if config['model'] == 'maml':  # need grad in maml
                    ...
                    with torch.no_grad():
                        logits, sslogits = model(x_shot, x_query, eval=True)
